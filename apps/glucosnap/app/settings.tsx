...
export default function Settings() {
  ...
  return (
    <ScrollView contentContainerStyle={[theme.screen, { padding: 16, gap: 12 }]}>
      ...
      <View style={{ alignItems: 'center', marginTop: 8, marginBottom: 16 }}>
        ...
        <Text style={[theme.text, { fontSize: 18, fontWeight: '700' }]}>{session?.name || 'Signed in user'}</Text>
...
function Row({ label, value }: { label: string; value?: string }) {
  return (
    <View style={{ paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f0f0f0', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
      <Text style={{ color: '#666' }}>{label}</Text>
      <Text style={{ fontWeight: '600', color: '#111' }}>{value || '—'}</Text>
    </View>
  );
}

...
async function getOpenAIKey(): Promise<string> {
  ...
  const resp = await ssm.send(new GetParameterCommand({ Name: OPENAI_API_KEY_SSM_PARAM, WithDecryption: true }));
  if (!resp.Parameter?.Value) throw new Error('Missing OpenAI API key in SSM');
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    const userId = (event.requestContext.authorizer as any)?.principalId || 'unknown';
    const email = (event.requestContext.authorizer as any)?.email || '';
    ...
    const getUrl = await getSignedUrl(s3, new GetObjectCommand({ Bucket: BUCKET_NAME, Key: key }), { expiresIn: 60 * 5 });
    ...
    await ddbDoc.send(new PutCommand({ TableName: RESULTS_TABLE, Item: item }));
...
function parseCarbs(text: string): number | null {
  try {
    ...
    if (jsonStart >= 0 && jsonEnd > jsonStart) {
      ...
      if (typeof obj.total_carbs_g === 'number') return obj.total_carbs_g;
    }
  } catch {}
